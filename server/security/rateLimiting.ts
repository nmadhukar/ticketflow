...
const aiApiCallTracker = new Map<string, { count: number; resetTime: number }>();
...
export const createRoleBasedRateLimit = (windowMs: number, limits: { customer: number, agent: number, admin: number }) => {
  return rateLimit({
    windowMs,
    max: (req: Request) => {
...
export const aiApiRateLimit = (req: AuthenticatedRequest, res: Response, next: Function) => {
  ...
  let tracker = aiApiCallTracker.get(userId);
  ...
  // Increment counter
  tracker.count++;
  aiApiCallTracker.set(userId, tracker);
...
export const cleanupAiApiTracker = () => {
  const now = Date.now();
  for (const [userId, tracker] of aiApiCallTracker.entries()) {
